...
const WindBarChart = ({ sensorData, selectedSensor, dateRange }) => {

    if (!sensorData || !sensorData.environmental)
        return <p>ไม่มีข้อมูล Sensor</p>;

    const windData = sensorData.environmental.find((entry) =>
        entry.param.toLowerCase().includes("wind_speed")
    );
    ...
    if (!windData || !windData.readings?.length) {
    ...
    const readings = useMemo(() => {
        if (!isDateRangeSelected) {
            ...
            return windData.readings.filter((r) => {
        ...
        return windData.readings;
    }, [windData.readings, isDateRangeSelected, dateRange]);
    ...
    }, [windData.readings, dateRange, isDateRangeSelected]);
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md h-[420px]">
            <div className="mb-4">
                <h2 className="text-lg font-bold text-gray-900 dark:text-white">ความเร็วลม</h2>
